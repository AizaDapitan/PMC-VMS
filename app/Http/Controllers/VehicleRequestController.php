...
class VehicleRequestController extends Controller
{
    ...
    public function tripTicket($id, Request $request){
        ...
        if(strlen($vehicle_request->date_needed) <= 14) { //example when '2020-11-01.000' this would cause Failed to parse time string
            ...
            $date_needed = Carbon::parse($date_needed[0])->format('Y-m-d 00:00:00');
        } else {
    ...
    public function createWithDestination(Request $request) {
        ...
        $vehicle_request->date_needed = date('Y-m-d H:i:s', strtotime($request->get('date_needed')));
        ...
        $vehicle_request->addedAt = date('Y-m-d H:i:s');
    ...
    public function update(Request $request){
        $vehicle_request = VehicleRequest::where('id', $request->id_edit)
            ->with([
                'requestOtherInfo'
            ])->first();
    ...
    public function editDispatchDetailsForm($id, Request $request)
    {
        ...
        try {
            ...
            return view('admin.requests.dispatch.dispatch_edit_form', 
    ...
    public function dispatchPrintout(Request $request, $id)
    {
        ...
        {
            $date_needed = Carbon::parse($vehicle_request[0]->date_needed)->format('Y-m-d h:i:s A');
        }
        else 
        {
            $date_needed = '';
        ...
        if($dispatch[0]->dateStart && strlen($dispatch[0]->dateStart) >= 14) {
            $date_start = Carbon::parse($dispatch[0]->dateStart)->format('Y-m-d h:i:s A');
        }
        else 
        {
            $date_start = '';
        ...
        $return_date = $dispatch[0]->dateEnd == NULL ? '____________________________' :  Carbon::parse($dispatch[0]->dateEnd)->format('Y-m-d h:i A');
